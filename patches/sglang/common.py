...
# RAGBoost integration settings
# Set RAGBOOST_INDEX_URL environment variable to enable sync with RAGBoost index
RAGBOOST_INDEX_URL = os.environ.get("RAGBOOST_INDEX_URL")
_ragboost_enabled = RAGBOOST_INDEX_URL is not None

if _ragboost_enabled:
    logger.info(f"RAGBoost integration enabled: {RAGBOOST_INDEX_URL}")
...
def evict_from_tree_cache(tree_cache: BasePrefixCache | None, num_tokens: int):
    ...
    if hasattr(allocator, "full_available_size"):
        ...
        if full_available_size < num_tokens or swa_available_size < num_tokens:
            ...
            # Eviction sync with RAGBoost is now handled via callback registered on RadixCache
    else:
        # Standard allocator
...
def create_ragboost_eviction_callback():
    """
    Create an eviction callback for syncing with RAGBoost index.
    
    This callback is invoked by RadixCache.evict() with the set of request_ids
    ...
    def eviction_callback(evicted_request_ids: set):
        ...
        # These are not tracked by RAGBoost and would cause "not_found" noise
        filtered_ids = {
        ...
        try:
            logger.info(f"[RAGBoost] Syncing eviction: {len(filtered_ids)} requests")
            requests.post(
