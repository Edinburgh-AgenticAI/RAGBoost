## RAGBoost Pipeline API

A high-level, user-friendly abstraction for building RAG pipelines with or without RAGBoost optimization.
...
- **Optional Optimization**: Easy toggle between RAGBoost and standard RAG
...
pipeline = RAGPipeline(
    ...
    corpus_path="corpus.jsonl",
    model="Qwen/Qwen2.5-7B-Instruct",
    use_ragboost=True,             # Enable RAGBoost
...
### Comparison: RAGBoost vs Standard RAG
...
# With RAGBoost
ragboost = RAGPipeline(
    retriever="bm25",
    corpus_path="corpus.jsonl",
    use_ragboost=True
)
ragboost_results = ragboost.run(queries)
...
# Compare
print(f"RAGBoost groups: {len(ragboost_results['optimized_batch'])}")
...
The pipeline is designed to integrate seamlessly with existing RAGBoost workflows:
...
- `optimizer`: OptimizerConfig or bool to enable/disable RAGBoost
...
- `use_ragboost`: Enable/disable RAGBoost (default: True)
- `**kwargs`: Additional configuration options

**Methods:**
- `setup()`: Initialize pipeline components
- `retrieve(queries, top_k)`: Retrieve documents for queries
- `optimize(retrieval_results)`: Apply RAGBoost optimization
...
- **OptimizerConfig**: Configure RAGBoost optimization
- **InferenceConfig**: Configure LLM inference
- **PipelineConfig**: Overall pipeline configuration

See `ragboost/pipeline/components.py` for detailed configuration options.
...
1. Simple RAGBoost pipeline with BM25
2. FAISS + RAGBoost
