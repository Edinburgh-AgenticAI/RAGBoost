...
Complete API documentation for RAGBoost.

## RAGPipeline

The main interface for RAGBoost.
...
pipeline = RAGPipeline(
    ...
    use_ragboost=True,             # Enable/disable RAGBoost optimization
...
| `use_ragboost` | bool | `True` | Enable RAGBoost optimization |
...
RAGBoost provides an HTTP server for live index management with SGLang integration.
...
```json
{
    ...
    "mode": "initial",
    "num_contexts": 2,
    "request_ids": ["ragboost_abc123", "ragboost_def456"],
    "stats": {...}
}
```
...
client = RAGBoostIndexClient("http://localhost:8765", timeout=1.0)
